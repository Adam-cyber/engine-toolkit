...
func TestNewRequestFromMediaChunk(t *testing.T) {
	...
	eng := Engine{}
	req, err := eng.newRequestFromMediaChunk(client, processSrv.URL, msg)
	is.NoErr(err)
...
func TestNewRequestFromMediaChunkDisableChunkDownload(t *testing.T) {
	is := is.New(t)
	fileSrv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	defer processSrv.Close()
	eng := Engine{}
	eng.Config.Processing.DisableChunkDownload = true
	req, err := eng.newRequestFromMediaChunk(client, processSrv.URL, msg)
	is.NoErr(err)
	resp, err := client.Do(req)
	is.NoErr(err)
