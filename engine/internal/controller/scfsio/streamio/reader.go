...
import (
	...
	qtfaststart "github.com/veritone/engine-toolkit/engine/internal/controller/stream-ingestor-v2/qt-faststart"
)
...
func StartStreamReader(ctx context.Context, cancel context.CancelFunc, sr Streamer, firstReadTimeout time.Duration) (*Stream, error) {

	var stream *Stream

	firstReadCh := make(chan struct{})
	go func() {
		defer close(firstReadCh)

		select {
		case <-time.After(firstReadTimeout):
			Logger.Printf("Haven't read anything for %v. Stop this task!\n", firstReadTimeout)
			if stream != nil {
				stream.SendErr(fmt.Errorf("timeout for the first read!"))
			}
			cancel()
			return
		case <-firstReadCh:
	...
	go func() {
		...
		logProgress := func() {
			...
			if !firstReadFlag && stream.bytesRead.Count() > 0 {
				firstReadCh <- struct{}{}
				firstReadFlag = true
			}
		}
