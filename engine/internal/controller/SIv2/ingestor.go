...
func (si *streamIngestor) WriteStream(ctx context.Context, stream *streamio.Stream) error {
	...
	go func() {
		for i, sw := range si.writers {
			...
			logger.Printf("Starting %q on stream %d", writerName, origStream.ID)
			...
			go func(sw streamio.StreamWriter) {
				...
				logger.Printf("%s finished (took %s)", writerName, time.Since(startTime))
			}(sw)
			...
			go func(ig ingest.Ingestor) {
				...
				for {
					...
					logger.Printf("Published %q object %d into chunk queue. [Offset: %d - %d ms] [URI: %s]",
	...
	for {
		...
		case err := <-errc:
			...
			if err != nil {
				...
				logger.Println("Published chunk_eof message:", string(chunkEOFJSON))
				return err
			}
