...
var jsonConfiguration = require("./config/service/settings.json")
var vtnAuthHelper = require('./vtnAuthHelper.js')
...
if (settings.useVeritoneServerAuth !== false) {
    ...
    app.use(session({
        ...
        secret: 'VERITONE_NODE_RED',
        cookie: {
            maxAge: 60000
        }
    }));
    // middleware component to verify authorized veritone user
    // app.use(vtnAuthHelper.verifyUser);
}

/************************************************************************************/

if (settings.https) {
    // settings.https.secureOptions = crypto.constants.SSL_OP_NO_TLSv1_2;
...
RED.start().then(function () {
    if (settings.httpAdminRoot !== false || settings.httpNodeRoot !== false || settings.httpStatic) {
        ...
        // add httpRequestTimeout to config/service/settings.json order to extend http request timeout
        server.timeout = jsonConfiguration.httpRequestTimeout || 120000;
    } else {
...
process.on('SIGINT', function () {
    console.log("Calling SIGINT and clean up");
    RED.stop().then(function () {
        process.exit();
    }).catch(e => console.log('exit failed for SIGINT', e));
});

// experiment for stop workflow
process.on('SIGTERM', function () {
    console.log("Calling SIGTERM and clean up");
    RED.stop().then(function () {
        process.exit();
    }).catch(e => console.log('exit failed for SIGTERM', e));
});
